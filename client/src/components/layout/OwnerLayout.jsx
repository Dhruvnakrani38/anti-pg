...
import { LayoutDashboard, Building2, Users, DollarSign, MessageSquare, User, LogOut, Menu, X } from 'lucide-react';
...
export default function OwnerLayout() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-100 flex flex-col shadow-xl md:shadow-sm transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Brand */}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile Header */}
        <header className="md:hidden bg-white border-b border-gray-100 h-16 flex items-center px-4 justify-between flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-primary-600 flex items-center justify-center text-white font-black text-xs">PG</div>
            <span className="font-bold text-gray-900 text-sm">Owner Panel</span>
          </div>
          <button 
